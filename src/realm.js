  ...
  pushPathConditionsLeadingToNormalCompletions(completion: Completion): void {
    ...
    if (completion instanceof JoinedAbruptCompletions || completion instanceof JoinedNormalAndAbruptCompletions) {
      if (completion.consequent.value === bottomValue || allPathsAreOfType(AbruptCompletion, completion.consequent)) {
        ...
        let cpc = AbstractValue.createFromLogicalOp(realm, "&&", jc, getNormalPathConditions(completion.consequent));
        let ijc = AbstractValue.createFromUnaryOp(realm, "!", jc);
        let apc = AbstractValue.createFromLogicalOp(realm, "&&", ijc, getNormalPathConditions(completion.alternate));
    ...
    function getNormalPathConditions(c: Completion): Value {
      ...
      if (c.consequent.value === bottomValue || allPathsAreOfType(AbruptCompletion, c.consequent)) {
        if (!allPathsAreOfType(AbruptCompletion, c.alternate)) {
          ...
          else pathCond = AbstractValue.createFromLogicalOp(realm, "&&", inverse, getNormalPathConditions(c.alternate));
        }
        ...
        if (!allPathsAreOfType(AbruptCompletion, c.consequent)) {
          if (allPathsAreOfType(NormalCompletion, c.consequent)) {
            ...
            pathCond = AbstractValue.createFromLogicalOp(realm, "&&", jc, getNormalPathConditions(c.consequent));
          }
        }
      } else {
        let jc = c.joinCondition;
        let consequentPC = AbstractValue.createFromLogicalOp(realm, "&&", jc, getNormalPathConditions(c.consequent));
        let ijc = AbstractValue.createFromUnaryOp(realm, "!", jc);
        let alternatePC = AbstractValue.createFromLogicalOp(realm, "&&", ijc, getNormalPathConditions(c.alternate));
        pathCond = AbstractValue.createFromLogicalOp(realm, "||", consequentPC, alternatePC);
      }
      ...
      return AbstractValue.createFromLogicalOp(realm, "&&", pathCondToComposeWith, pathCond);
    }
  }
